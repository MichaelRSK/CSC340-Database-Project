...
- [x] Build `dashboard_frame.py` — base layout with sidebar navigation
- [x] Build `manager_frame.py` — manager view: shows all nav items (Dashboard, Products, Categories, Suppliers, Inventory, Customers, Orders, Returns)
- [x] Build `employee_frame.py` — employee view: limited nav (Dashboard, Orders, Inventory only)
- [x] Wire role routing: after login, load correct frame based on `role` from DB
- [x] Add logout button → clears session, returns to login screen
- [x] Commit: `Added role-based dashboard: manager and employee views`
